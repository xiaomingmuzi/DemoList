...
public class HttpTask<ResultType> extends AbsTask<ResultType> implements ProgressHandler {
    ...
    public HttpTask(RequestParams params, Callback.Cancelable cancelHandler,
        ...
        super(cancelHandler);
        params2 = params.getStringParams();
        assert params != null;
    ...
    protected ResultType doBackground() throws Throwable {
        ...
        if (cacheCallback != null && HttpMethod.permitsCache(params.getMethod())) {
            ...
            try {
                clearRawResult();
                LogUtils.d("load cache: " + this.request.getRequestUri());
                rawResult = this.request.loadResultFromCache();
            } catch (Throwable ex) {
                LogUtils.w("load disk cache error", ex);
            }
            ...
            if (rawResult != null) {
                if (prepareCallback != null) {
                    try {
                        ...
                        cacheResult = null;
                        LogUtils.w("prepare disk cache error", ex);
                    } finally {
                        clearRawResult();
                    }
                } else {
        ...
        while (retry) {
            ...
            try {
                ...
                try {
                    clearRawResult();
                    // 开始请求工作
//                    LogUtils.d("load: " + this.request.getRequestUri());
                ...
                if (prepareCallback != null) {
                    ...
                    try {
                        ...
                        String param=params2.size()!=0? "\n Param ==》" + params2.toString() : "";
                        LogUtils.i("请求的url: " + this.request.getRequestUri() + param + "\n返回结果==》" + getNewResult((String) result));
                    } finally {
                        clearRawResult();
                    }
                } else {
                ...
                retry = true;
                LogUtils.w("Http Redirect:" + params.getUri());
            } catch (Throwable ex) {
    ...
    private String getNewResult(String theString) {
        char aChar;
