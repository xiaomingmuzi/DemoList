...
//        LogUtils.w(ex.getMessage(), ex);

        if (count > maxRetryCount || request == null) {
            LogUtils.w("The Max Retry times has been reached!");
            return false;
        }

        if (!HttpMethod.permitsRetry(request.getParams().getMethod())) {
            LogUtils.w("The Request Method can not be retried.");
            return false;
        }

        if (blackList.contains(ex.getClass())) {
            LogUtils.w("The Exception can not be retried.");
            return false;
        }
