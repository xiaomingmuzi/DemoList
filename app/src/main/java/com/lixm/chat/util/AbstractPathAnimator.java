...
public abstract class AbstractPathAnimator {
    ...
    public Path createPath(AtomicInteger counter, View view, int factor) {
        ...
        y2 = y - y2 / 2;
        Path p = new Path();
        p.moveTo(mConfig.initX, y);//起点
        LogUtils.e("======三次贝塞尔曲线控制点坐标（x,y）==》（" + mConfig.initX + "," + (y - factor) + ")   (x1,y1)==>（" + x + "," + (y2 + factor) + "）  终点 （" + x + "," + y2 + "）  " +
                "\n factor " + factor + "    mConfig.bezierFactor  " + mConfig.bezierFactor);
        p.cubicTo(mConfig.initX, y - factor, x, y2 + factor, x, y2);//绘制三次贝塞尔曲线
        p.moveTo(x, y2);//从上一次的终点开始，进行第二次贝塞尔曲线的绘制
        p.cubicTo(x, y2 - factor, x2, y3 + factor, x2, y3);//绘制第二条三次贝塞尔曲线
        return p;
    }
    ...
    public static class Config {
        ...
        static public Config fromTypeArray(TypedArray typedArray, float x, float y, int pointx, int heartWidth, int heartHeight) {
            ...
            config.bezierFactor = typedArray.getInteger(R.styleable.HeartLayout_bezierFactor, 6);//默认是6
            config.xPointFactor = pointx;
            config.heartWidth = heartWidth;
            config.heartHeight = heartHeight;
            config.animDuration = typedArray.getInteger(R.styleable.HeartLayout_anim_duration, 2000);//持续期
            return config;
        }
